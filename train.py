...
from models import VNet, MultiPeriodDiscriminator, MultiResolutionDiscriminator,\
...
def train(rank, a, h):
    print('here-------------')
    if h.num_gpus > 1:
        #torch.distributed.init_process_group(backend='nccl', init_method='tcp://localhost:12345',
        #                                 world_size=2, rank=rank)
        #torch.cuda.set_device(rank)
        # initialize distributed初始化分布式训练环境
    ...
    # define VNet generator
    generator = VNet(h).to(device)
    ...
    # define additional discriminators. VNet uses MRD as default
    ...
    if rank == 0:#如果当前进程的排名是0，打印生成器的结构，并在指定的路径创建检查点目录。
        # print(generator)
        os.makedirs(a.checkpoint_path, exist_ok=True)
        print("checkpoints directory : ", a.checkpoint_path)

    if os.path.isdir(a.checkpoint_path):#检查指定的检查点目录是否存在，如果存在，则扫描最新的生成器检查点和鉴别器检查点
        cp_g = scan_checkpoint(a.checkpoint_path, 'g_')
        cp_do = scan_checkpoint(a.checkpoint_path, 'do_')

    # load the latest checkpoint if exists
    steps = 0#根据检查点的存在与否，加载生成器和鉴别器的状态。如果检查点不存在，设置state_dict_do = None，last_epoch = -1
    if cp_g is None or cp_do is None:
        state_dict_do = None
        last_epoch = -1
    else:#如果存在，加载生成器和鉴别器的状态字典，并更新步数和最后一个训练周期的值
    ...
    for d in (mrd.module if h.num_gpus > 1 else mrd).discriminators:
        d.conv_post.normalize_weight()#对判别器中的卷积层进行权重归一化操作
    ...
    #对模型进行验证，并计算验证集上的评估指标，如Mel频谱误差、PESQ和MRSTFT。它还提供了保存音频和频谱图的选项，以及将评估指标写入TensorBoard的功能
...
def main():
    ...
    parser.add_argument('--checkpoint_path', default='exp/vnet')
